...
def main():
    ...
    marks = range(11)
    num_unique_answers = sum(1 for answers in marks_distribution if answers)
    cmap = plt.get_cmap('rainbow', num_unique_answers + 1)  # Ensure enough colors
    colors = [cmap(i) for i in range(num_unique_answers)]
    bottom = np.zeros(11)
    for i, answers in enumerate(marks_distribution):
        if answers:
            bar_labels = [f"{answer}" for answer in answers]
            plt.bar(marks, [len(answers)] * 11, bottom=bottom, color=colors[i], edgecolor='white', label=', '.join(bar_labels))
            bottom += np.array([len(answers)] * 11)
...
def main():
    ...
    for i, answers in enumerate(marks_distribution):
        if answers:
            bar_labels.extend([f"{answer}" for answer in answers])
            ax.bar(marks, [len(answers)] * 11, bottom=bottom, color=colors[i], edgecolor='white', label=', '.join(answers))
            bottom += np.array([len(answers)] * 11)
