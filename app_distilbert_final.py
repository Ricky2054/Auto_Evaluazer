...
# Preprocessing
def preprocess_distilbert(text):
    tokens = tokenizer.tokenize(text)
    tokens = tokens[:512]  # DistilBERT max input length
...
def evaluate_long_answer(answer1, answer2, model, sentiment_model, max_seq_length=512):
    tokens_tensor1 = preprocess_distilbert(answer1)
    tokens_tensor2 = preprocess_distilbert(answer2)
    ...
    with torch.no_grad():
        output1 = model(tokens_tensor1_padded, attention_mask=attention_mask1_padded)
        last_hidden_states1 = output1.last_hidden_state
        output2 = model(tokens_tensor2_padded, attention_mask=attention_mask2_padded)
